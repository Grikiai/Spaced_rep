...
class GameDay:
    ...
    def overall_progress(self):
        if self.get_progress().strip().isalpha():
            ...
            achieved = 'on %s session you made no progress, since all of the boxes were empty' (self.date_now)
            progress.write()
            progress.close()
        else:
            progress = open('progress.txt', 'a')
            achieved = 'on %s session you answered %s precent correctly' (self.date_now, self.get_progress())
            progress.write()
            progress.close()
